...
var sel_option = document.getElementById("select-option");

sel_option.addEventListener("change", function() {
  const sv_item = document.getElementById("canvas");
  let val = sel_option.value;

  if(val=="6-4-notecard"){
    sv_item.style.width = "576px";


}else if(val=="svg"){
    sv_item.style.width = "100vh";
    
}else if(val=="no-10-envelope"){
  sv_item.style.width = "912px";
...
ur = "https://d33au9p5d8tjhf.cloudfront.net/d.bin",
  ...
  ar("save-button").addEventListener("click", () => {
    ...
    l.setAttribute("href", "data:image/svg+xml;base64," + window.btoa(a)),
      ...
      l.click()
  }else if(option=="6-4-notecard"){

    convert(576, 384);

  }else if(option=="no-10-envelope"){
  ...
  function convert(width, height) {
    var r = ar("canvas").getBBox();
    var e = [hr(r.x - 3), hr(r.y - 3), width, height].join(" "); // Set the width and height of the viewBox
    var t = ar("canvas");
    t.setAttribute("viewBox", e);
    var a = (new XMLSerializer).serializeToString(t);
    t.removeAttribute("viewBox");
    ...
    img.onload = function() {
      ...
      // Set the canvas dimensions to the custom dimensions
      canvas.width = width;
      ...
      // Create a new anchor element (a), set its href attribute to the data URL of the image,
      // set its download attribute to the desired filename, and programmatically click it
      var link = document.createElement('a');
      link.href = dataUrl;
      link.download = Z.toString().toLowerCase().replace(/\s+/g, "-").replace(/[^\w\-]+/g, "").replace(/\-\-+/g, "-").trim() + ".png";
    ...
    img.src = 'data:image/svg+xml;base64,' + window.btoa(a);
  }
